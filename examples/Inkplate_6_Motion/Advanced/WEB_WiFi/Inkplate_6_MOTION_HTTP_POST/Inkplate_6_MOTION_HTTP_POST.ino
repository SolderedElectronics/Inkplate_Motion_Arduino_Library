 ...
 * @brief       This example will show you how to make a POST request via WiFi
 *
...
// The URL which we're making POST to
#define POST_URL "https://httpbin.org/post"
// The https:// part is important in this case!
...
void setup()
{
    ...
    inkplate.print("Making POST request to ");
    inkplate.print(POST_URL);
    inkplate.println("...");
    inkplate.partialUpdate(true);
    delay(1000); // Wait a bit

    // Do POST and print the results, details in this function
    makePost();
...
// Simple function to make POST request and print the results to Inkplate
void makePost()
{
    ...
    // Try to connect to the server to POST to
    if (client.begin(POST_URL))
    {
        // Create request body
        char bodyStr[256];
        sprintf(bodyStr, "{\"message\":\"Hi Inkplate!\"}");

        if (client.POST(bodyStr, strlen(bodyStr)))
        {
            inkplate.println("Received back:");
        ...
        {
            inkplate.println("Failed to make POST request!");
        }
