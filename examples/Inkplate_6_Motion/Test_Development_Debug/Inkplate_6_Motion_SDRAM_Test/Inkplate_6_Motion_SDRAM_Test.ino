...
bool testSDRAMChunk(uint32_t _startAddress, uint32_t _endAddress, uint64_t *_writeSpeed, uint64_t *_readSpeed, uint16_t *_failIndex)
{
    ...
    for (int i = 0; i < _len; i++)
    {
        ...
        {
            if (_failIndex != NULL) *_failIndex = i;
            return false;
        }
    }
